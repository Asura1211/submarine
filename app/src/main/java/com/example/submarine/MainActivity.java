...
public class MainActivity extends BaseActivity implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //为碰撞检测服务
        fgLayout.setBgLayout(bgLayout);
        //restart按钮不显示
        findViewById(R.id.restart).setVisibility((int)8);
...
//                long start = System.currentTimeMillis();
                ...
                if (faceRectView != null && drawHelper != null) {
                    ...
                    for (int i = 0; i < faceInfoList.size(); i++) {
                        Rect rect = drawHelper.adjustRect(faceInfoList.get(i).getRect());
                        drawInfoList.add(new DrawInfo(rect, genderInfoList.get(i).getGender(), ageInfoList.get(i).getAge(), faceLivenessInfoList.get(i).getLiveness(), RecognizeColor.COLOR_UNKNOWN, null));
            ...
            timer.schedule(new TimerTask() {
                ...
                public void run() {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            if(fgLayout.boatView.isDead)
                                findViewById(R.id.restart).setVisibility((int)0);
                                //reStart();
                        }
                    });
                }
